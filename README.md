...
Live Link: https://huggingface.co/spaces/MFH-001/MediScan-AI
...
> **Related project:** [PsoriScan AI](https://github.com/mfh-001/PsoriScan-AI) — a standalone psoriasis plaque segmentation and PASI-inspired severity scoring tool, built as a focused extension of the medical imaging work started here.
...
To isolate skin lesions, the system implements a U-Net architecture. It processes 256×256 RGB images, producing a binary mask where each pixel is classified as 'Lesion' or 'Background'. The lesion severity is quantified by:
 
```
Area Percentage = (Σ Pixels_mask / Σ Pixels_total) × 100
```
...
- **Text Normalization:** Custom fuzzy matching with a **75% similarity threshold** to handle common medical misspellings (e.g., "diabetis" → "diabetes").
...
The live Hugging Face Spaces version runs the **knowledge base mode** — the structured medical dictionary handles Q&A without requiring GPU resources. This allows free, permanent hosting with sub-second response times on CPU.
 
The full Llama 3.2 3B Instruct pipeline (as built in the training notebook) requires a GPU with ~6GB VRAM.
...
- `aap-HF.py` — Containing Hugging Face live version
- `requirements.txt` — List of necessary dependencies.
- `MediScan AI - Demo Video.mov` — A walkthrough of the functional application.
